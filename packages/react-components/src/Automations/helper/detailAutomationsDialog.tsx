...
const DetailAutomationsDialog: FC<DetailAutomationsDialogProps> = ({ open, onClose, automation }) => {
  return (
    <Dialog open={open} onClose={onClose} maxWidth='xl'>
      <Paper elevation={3} style={{ padding: '10px', overflow: 'hidden' }}>
        <DialogTitle>
          ...
          <Box>
            ...
            <Paper style={{ padding: '15px' }}>
              <Box style={{ marginBottom: '15px' }}>
                <Typography variant='h6' style={{ lineHeight: '2' }}>Job Trigger List</Typography>
              ...
              <Box style={{ maxHeight: '300px', overflowY: 'auto' }}>
                {automation?.triggerCondition?.triggers.map((trigger) =>
                  <Box key={trigger.id} sx={{ border: '1px solid lightgray', borderRadius: '10px', padding: '10px', marginBottom: '10px' }}>
