...
const TimeseriesExporter: FC<ITimeseriesExporterProps> = (
  ...
  const exportTable = () => {
    ...
    timesteps = timesteps.filter(
      (value, index, self) => self.indexOf(value) === index
    );

    timesteps = timesteps.map(timestep =>
      format(parseISO(timestep), "yyyy-MM-dd'T'HH:mm:ss")
    );
    ...
    timesteps.forEach((datetime: string) => {
      const cells = [
        format(
          parseISO(datetime),
          props.dateTimeFormat || 'yyyy-MM-dd HH:mm:ss'
        ),
      ];

      const dateTimeLookup = format(
        parseISO(datetime),
        "yyyy-MM-dd'T'HH:mm:ss"
      );

      props.data.forEach((timeseries: ITimeseriesData, index: number) => {
        const found = timeseries.data.filter(
          (timestep: (string | number)[]) => timestep[0] === dateTimeLookup
        );
    ...
    link.setAttribute(
      'download',
      !props.exportFileName ? 'Export.csv' : `${props.exportFileName}.csv`
    );
