...
import { UserGroupsDX } from './UserGroupsDX';
...
export const UserGroupDevExpressStory = () => {
  const metadata = [
    {
      key: 'myChoice',
      label: 'My Choice',
  ...
  return (
    <LoginGate host={host} showRememberMe={true} textFieldVariant={'outlined'}>
      {({ token }) => <UserGroupsDX host={host} metadata={metadata} token={token.accessToken.token} />}
    </LoginGate>
  );
};
